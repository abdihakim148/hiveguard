...
/// - `owners`: Maps the secondary key (SK) to the primary key (PK) for
///   quick lookup of verification records by owner.
/// - `verifications`: Maps the primary key (PK) to the `Verification`
///   item, allowing retrieval and management of verification data.
pub struct Verifications {
    owners: RwLock<HashMap<<Verification as Item>::SK, <Verification as Item>::PK>>,
    verifications: RwLock<HashMap<<Verification as Item>::PK, Verification>>,
}
...
impl Table for Verifications {
    ...
    async fn create(&self, item: &Self::Item) -> Result<<Self::Item as Item>::PK, Self::Error> {
        let id = item.id;
        let (mut owners, mut verifications) = (self.owners.write()?, self.verifications.write()?);
        let res = (owners.insert(item.owner_id, item.id), verifications.insert(item.id, item.clone()));
        Ok(id)
    }
    ...
    async fn get(&self, key: Either<&<Self::Item as Item>::PK, &<Self::Item as Item>::SK>) -> Result<Option<Self::Item>, Self::Error> {
        ...
        Ok(verifications.get(pk).cloned())
    }
    ...
    async fn delete(&self, id: &<Self::Item as Item>::PK) -> Result<(), Self::Error> {
        let (mut verifications, mut owners) = (self.verifications.write()?, self.owners.write()?);
        let verification = match verifications.remove(id) {
            Some(verification) => verification,
            None => return  Ok(())
        };
        owners.remove(&verification.owner_id);
        Ok(())
    }
}
...
mod tests {
    ...
    async fn test_create_and_get_verification() {
        let verifications = Verifications::new().await.unwrap();
        let verification = Verification::default();

        // Test creation
        let pk = verifications.create(&verification).await.unwrap();
        assert_eq!(pk, verification.id);

        // Test retrieval by primary key
        let retrieved = verifications.get(Either::Left(&verification.id)).await.unwrap();
        assert_eq!(retrieved, Some(verification.clone()));

        // Test retrieval by secondary key
        let retrieved_by_sk = verifications.get(Either::Right(&verification.owner_id)).await.unwrap();
    ...
    async fn test_delete_verification() {
        ...
        verifications.create(&verification).await.unwrap();
        verifications.delete(&verification.id).await.unwrap();
